...
<?
    ...
    for($cont=1;$cont<=$numero_campos;$cont++)
    {
      ...
      {
        echo "<br>";
        edit_senha($campos[$cont2],$campos[$cont1],$campos[$cont4]);  
      }
	  
      if($campos[$cont3]=="senha_criptografada")
      {
        echo "<br>";
        edit_senha_md5($cont,$campos[$cont2],$campos[$cont1],$campos[$cont4]);  
      }

      if($campos[$cont3]=="inteiro")
      {
        echo "<br>";
        edit_inteiro($campos[$cont2],$campos[$cont1],$campos[$cont4],$campos[$cont10]);  
      }

      if($campos[$cont3]=="moeda")
      {
        echo "<br>";
        edit_moeda($campos[$cont2],$campos[$cont1],$campos[$cont4],$campos[$cont10]);  
      }

      if($campos[$cont3]=="real")
      {
        echo "<br>";
        edit_real($campos[$cont2],$campos[$cont1],$campos[$cont4],$campos[$cont10]);  
      }
      ...
      {
        echo "<br><br>";
        editor_edit($campos[$cont2],$campos[$cont1],$campos[$cont4]);  
      }

      if(($campos[$cont3]=="data_int")||($campos[$cont3]=="data_int_now"))
      {
        echo "<br>";
        data_int_edit2($campos[$cont2],$campos[$cont1],$campos[$cont10]);  
      }
      ...
      {
        data_hora_editar($campos[$cont2],$campos[$cont1]);  
      }
      ...
      if($campos[$cont3]=="chave_estrangeira")      
      {
        ...
        select_edit_ajax($campos[$cont2],$campos[$cont6],$campos[$cont5],$campos[$cont7],$campos[$cont1],$sistema_exclusao,$campos[$cont10],$cont,$modulo);  
      }
      ...
      if($campos[$cont3]=="usuario")      
      {
        ...
        select_usuario_edit($campos[$cont2],$campos[$cont6],$campos[$cont5],$campos[$cont7],$campos[$cont1],$sistema_exclusao);
      }
      ...
      if($campos[$cont3]=="chave_estrangeira_recursiva")
      {
        ...
        echo "<br><br>";
        select_recursivo_edit($campos[$cont2],$campos[$cont6],$campos[$cont5],$campos[$cont7],$campos[$cont1],$sistema_exclusao,$campos[$cont10]);
      }
    ...
    for($conta=1;$conta<=$numero_campos_associativos;$conta++)
    {
      ...
      echo "<br><br>";
      select_multi_edit2($campos_associativos[$conta2],$campos_associativos[$conta6],$campos_associativos[$conta5],$campos_associativos[$conta7],$campos_associativos[$conta8],$campos_associativos[$conta9],$sistema_exclusao);
    }
    ...
    for($y=1;$y<=$num_sistemas_relacionados_deste;$y++)
    {
      ...
      while($linha_relacionado = mysql_fetch_array($rs_relacionados))
      {
        ...
        echo '<td><a style="text-decoration:none; font-size:12px; color:#000000;" href="' . $link_exclusao . '">[excluir]</a></td>
		';
      ...
      for($x=1;$x<=$numero_campos_relacionados;$x++)
      {
        ...
        if(($campo_relacionado[$x1]!=$chave_primaria_original)&&($campo_relacionado[$x3]!="chave_primaria"))
        {
          ...
          if(($campo_relacionado[$x3]=="data_int")||($campo_relacionado[$x3]=="data_int_now"))
          {
            echo input_data_int_relacionado($campo_relacionado[$x1],$campo_relacionado[$x2],$indice);  
            $indice++;
          }
    ...
    else
    {
      ...
      {  
        echo '<br><br><a class="caminho" href="editar_documento.php?codigo_modulo=' . $codigo_modulo . '&pagina=' . $pagina . '&' . $chave_primaria_original . '=' . anti_injection($_REQUEST[$chave_primaria_original]) . '"><b>>> Gerenciar Documentos</b></a>';

        if($tipo_sistema_documentos_original==2)
        {  
          echo '<br><br><a class="caminho" href="../_upload_documentos_lote/formulario.php?up_codigo_modulo=' . $codigo_modulo . '&' .$chave_primaria_original . '=' . anti_injection($_REQUEST[$chave_primaria_original])  . '"><b>>> Enviar Documentos em Lote</b></a>';
        }
      }
    }
