...
// exit();
  ...
  for($y=1;$y<=$num_sistemas_relacionados_deste;$y++)
  {
    ...
    foreach($sql_atributo_novo as $atributo_novo)
    {
      ...
      if($atributo_novo!="")
      {
        ...
        // PEGA NOVO C�DIGO
        $sql = "SELECT * FROM " . $tabela_relacionado . " ORDER BY " . $chave_primaria_relacionado . " DESC LIMIT 1";
        $rs_novo_codigo = mysql_query($sql,$conexao)or die (mysql_error());
        $linha_novo_codigo = mysql_fetch_array($rs_novo_codigo);
        $novo_id = $linha_novo_codigo[$chave_primaria_relacionado] + 1;


        $sql = " INSERT INTO " . $tabela_relacionado . "(" . $chave_primaria_relacionado . "," .$chave_primaria_original.$campos_relacionados_para_sql.")";
        $sql.= " VALUES ('" . $novo_id. "','" . $_REQUEST[$chave_primaria_original] . "','" . $atributo_novo . "')";
  ...
  mysql_query($sql,$conexao);
    ...
    {
      if((trim($_POST['adicionar']))=="+")
      {
        header("Location: editar_dados.php?codigo_modulo=" . $codigo_modulo . "&".$chave_primaria_original."=" . $_REQUEST[$chave_primaria_original] . "&pagina=" . $_REQUEST['pagina']);  
      }
      else
      {
