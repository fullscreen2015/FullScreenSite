...
if (ISSET($_SESSION['senha_result']) && ISSET($_SESSION['login_result']))
{
  ...
  $permissao = 3;
	...
	if(verifica_usuario2($codigo_modulo, $permissao))
	{
      ...
      $exibir_este_campo = true;

      if(ISSET($campos[$cont15]))
      {
      	if($campos[$cont15]=="1")
      	{
      		$exibir_este_campo = false;
      	}
      }

      if($exibir_este_campo)
      {
	      ...
	      if(ISSET($campos[$cont14]))
	      {
	        $help14 = $campos[$cont14];
				...
				{
					echo "<br>";
					input_senha($campos[$cont2],$campos[$cont1],$campos[$cont4]);  
				}
				...
				{
					echo "<br>";
					input_inteiro($campos[$cont2],$campos[$cont1],$campos[$cont4],$campos[$cont10]);  
				}

				if($campos[$cont3]=="moeda")        
				{
					echo "<br>";
					input_moeda($campos[$cont2],$campos[$cont1],$campos[$cont4],$campos[$cont10]);  
				}

				if($campos[$cont3]=="real")        
				{
					echo "<br>";
					input_real($campos[$cont2],$campos[$cont1],$campos[$cont4],$campos[$cont10]);  
				}
				...
				{
					echo "<br>";
					radio($campos[$cont2],$campos[$cont1]);  
				}
				...
				{
					echo "<br><br>";
					editor($campos[$cont2],$campos[$cont1],$campos[$cont4]);  
				}
				...
				{
					echo "<br>";
					data2($campos[$cont2],$campos[$cont1],$campos[$cont10]);  
				}
				...
				{
					data_hora_inserir($campos[$cont2],$campos[$cont1]);  
				}
				...
				if($campos[$cont3]=="usuario")      
				{
					...
					echo "<br><br>";
					select_usuario($campos[$cont2],$campos[$cont6],$campos[$cont5],$campos[$cont7],$campos[$cont1],$sistema_exclusao);
				}
				...
				if($campos[$cont3]=="chave_estrangeira")      
				{
					...
					echo "<br><br>";
					select2($campos[$cont2],$campos[$cont6],$campos[$cont5],$campos[$cont7],$campos[$cont1],$sistema_exclusao,$campos[$cont10]);
				}
				...
				if($campos[$cont3]=="chave_estrangeira_recursiva")      
				{
					...
					echo "<br><br>";
					select_recursivo($campos[$cont2],$campos[$cont6],$campos[$cont5],$campos[$cont7],$campos[$cont1],$sistema_exclusao,$campos[$cont10]);
				}
				...
				{
					echo "<br><br>";
					select_multi2($campos[$cont2],$campos[$cont6],$campos[$cont5],$campos[$cont7],$sistema_exclusao);
				}
		...
		for($conta=1;$conta<=$numero_campos_associativos;$conta++)
		{
			...
			select_multi2($campos_associativos[$conta2],$campos_associativos[$conta6],$campos_associativos[$conta5],$campos_associativos[$conta7],$sistema_exclusao);
		}
		...
		for($x=1;$x<=$numero_campos_interlacados;$x++)
		{
			...
			while($linha_tabela_pai=mysql_fetch_array($rs_tabela_pai))
			{
				...
				select2($linha_tabela_pai[$tabela_pai_campo_descricao],$sql_tabela_filho,$tabela_filho_chave_primaria,$tabela_filho_campo_descricao,$tabela_associativa_nome_campo_filho . "[]",$tabela_filho_exclusao,$obrigatorio);
		...
		$sf = 1; //sistema fotos relacionadas CRIADO PARA SOLUCIONAR ERRO/ O CERTO DEVE FUNCIONAR ASSIM  foto_relacionado_upload_1 / foto_relacionado_upload_2 / foto_relacionado_upload_3
		//E N�O PEGANDO O $Y
		...
		for($y=1;$y<=$num_sistemas_relacionados_deste;$y++)
		{
			...
			for($x=1;$x<=$numero_campos_relacionados;$x++)
			{
				...
				if(($campo_relacionado[$x1]!=$chave_primaria_original)&&($campo_relacionado[$x3]!="chave_primaria"))
				{
					...
					if($campo_relacionado[$x3]=="chave_estrangeira")
					{
						...
						$tabela_atributos.= select_relacionado($campo_relacionado[$x2],$campo_relacionado[$x6],$campo_relacionado[$x5],$campo_relacionado[$x7],$sistema_exclusao);  
					}
			...
			if($sistema_fotos_relacionado==1) // " $sf++ " SOMENTE PARA O SISTEMA DE FOTOS RELACIONADAS/ ALTERA��O FEITA PARA CORRE��O ANTES PEGAVA O $y e não dava a sequencia certa para modulode foto ralacionado
			// foto_relacionado_upload_1 / foto_relacionado_upload_2 / foto_relacionado_upload_3
			{
				$tabela_atributos.= "<td><input type=file name=foto_relacionado_upload_" . $sf++ . "[]></td>";
			}
