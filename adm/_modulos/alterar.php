<?php
  ...
  if (ISSET($_SESSION['senha_result']) && ISSET($_SESSION['login_result']))
  {
    ...
    if(isset($_REQUEST['codigo_modulo']))
    {
    ...
    if($codigo_modulo=="")
    {
      echo "C�digo do M�dulo não informado. Acesso negado.";
      exit();
    }
    ...
    if((verifica_usuario2($codigo_modulo, $permissao))&&($mostrar_editar==true))
    {
      ...
      /* =================== ATUALIZA DADOS RELACIONADOS ================================================== */
        ...
        if(isset($_POST[$nome_form_campo_relacionado_atual]))
        {
          ...
          // echo $nome_form_campo_relacionado_atual . " - " . $campo_relacionado[$x3] . "<br>";

          if($campo_relacionado[$x3] == "data_int")
          {
              $partes = explode("_rel",$nome_form_campo_relacionado_atual);

              $nome_campo_data_relacionado = $partes[0];
              
              if(isset($_POST[$nome_campo_data_relacionado . "_dia_rel"]))
              {
                // echo '<pre>';
                // print_r($_POST[$nome_campo_data_relacionado . "_dia_rel"]);
                // echo '</pre>';
              
                for ($contador=0; $contador < count($_POST[$nome_campo_data_relacionado . "_dia_rel"]) ; $contador++) 
                { 
             
                  $data_1[$contador] = $_POST[$nome_campo_data_relacionado . "_dia_rel"][$contador];
                  $data_2[$contador] = $_POST[$nome_campo_data_relacionado . "_mes_rel"][$contador];
                  $data_3[$contador] = $_POST[$nome_campo_data_relacionado . "_ano_rel"][$contador];
             
                  $_POST[$nome_form_campo_relacionado_atual][$contador] = zerosaesquerda($data_3[$contador],4) . zerosaesquerda($data_2[$contador],2) .  zerosaesquerda($data_1[$contador],2); 
                }
                // echo '<pre>';
                // print_r($_POST[$nome_form_campo_relacionado_atual]);
                // echo '</pre>';
              }
          }
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if(array_sum($atributos_valores) >0)
        {
          ...
          // adicionando a condi��o para atualizar somente registros da chave primaria

          $sql_update .= " AND " . $chave_primaria_original . "=" . $_REQUEST[$chave_primaria_original]; 
        ...
        for($cont=1;$cont<=$numero_campos;$cont++)
        {
          ...
          if(ISSET($_POST[$nome_do_campo_relacionado]))
          {
            ...
            foreach($_POST[$nome_do_campo_relacionado] as $valor_relacionado)
            {
              ...
              if(file_exists("personalizado_alterar_relacionado_6.php"))
              {
                include("personalizado_alterar_relacionado_6.php");
              }
