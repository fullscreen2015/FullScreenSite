<?php
  ...
  $campo_principal[111]="p.codigo_pedido";
  $campo_principal[112]="tabela_ec_pedidos_detalhes p, tabela_ec_pedidos_itens pi, tabela_ec_clientes_enderecos en, tabela_ec_clientes_detalhes c, tabela_ec_tipos_envio";
  $campo_principal[113]="n_pedidos";
  
  
  //Condi��o para Filtragem
  
   $campo_filtro[111]=" p.codigo_cliente = c.codigo_cliente AND p.codigo_endereco = en.codigo_endereco AND p.codigo_pedido = pi.codigo_pedido AND p.codigo_situacao = 4 AND p.ativo = 1"; 
  ...
  $sql3 = " SELECT DISTINCT(" . $campo_principal[111] . ")";
  ...
  $campo_registro = explode(".",$campo_principal[111]);
  $nome_campo = $campo_registro[1];

  $t=0;
  while($linha_registros = mysql_fetch_array($rs_registros))
  {
    $registros[$t] = $linha_registros[$nome_campo] . "<br>";
    $t++;
  }
  ...
  $nome_relatorio = "Pedidos Expedidos";

  // Campos do Grupo e Itens

  $sql = " SELECT p.codigo_pedido, descricao_tipo_envio, p.numero_pedido, p.valor_pedido, p.codigo_cliente, p.session, p.data_pedido, p.codigo_endereco,";
  $sql.= " pi.codigo_item, pi.descricao_item, pi.codigo_pedido, pi.quantidade, pi.valor_unitario, ";
  ...
  $sql.= " FROM tabela_ec_pedidos_detalhes p, tabela_ec_pedidos_itens pi, tabela_ec_clientes_enderecos en, tabela_ec_clientes_detalhes c, tabela_ec_tipos_envio";
  ...
  while(($linha = mysql_fetch_array($rs_dados)) && ($registros<$registros_por_pagina))
  {	
