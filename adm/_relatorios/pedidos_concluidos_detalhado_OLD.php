<?php
  ...
  $sql = " SELECT DISTINCT(p.codigo_pedido), descricao_tipo_envio,";
  $sql.= " p.numero_pedido, p.valor_pedido, p.codigo_cliente, p.codigo_situacao, p.session, p.data_pedido, p.codigo_endereco, p.valor_frete, p.valor_total, ";
  ...
  if($relatorio_com_paginacao==1)
  {
    ...
    $registro_inicial = ($pagina-1)*$registros_por_pagina; 

    $sql.= " LIMIT " . $registro_inicial . "," . $registros_por_pagina;
echo $sql;
// exit();
