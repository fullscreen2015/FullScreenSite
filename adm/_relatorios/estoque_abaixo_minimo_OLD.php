<?php
  ...
  $sql = " SELECT DISTINCT(p.codigo_produto),p.referencia_produto,p.descricao_produto,p.texto_produto,p.peso_produto,p.codigo_categoria,";
  $sql.= " p.acessos, p.publicar, p.ativo, c.descricao_categoria, e.* FROM tabela_ec_produtos_detalhes p, tabela_ec_produtos_categorias c, tabela_ec_produtos_estoque e";
  $sql.= " WHERE ";
  $sql.= " p.codigo_categoria = c.codigo_categoria AND";
  $sql.= " e.quantidade <= e.quantidade_minima AND";
  $sql.= " e.codigo_produto = p.codigo_produto AND";
  $sql.= " p.ativo = 1 AND";
  $sql.= " p.publicar = 1";
