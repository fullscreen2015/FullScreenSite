<?php
  ...
  //Condi��o para Filtragem
   ...
   $campo_filtro[111]="codigo_situacao=2 AND codigo_forma_de_pagamento=4 AND ativo=1 AND data_pedido < (now() - INTERVAL 3 DAY)"; 
  ...
  $nome_relatorio = "Vendas por boleto em aberto h� mais de três dias";


  // Campos do Grupo e Itens

  $dia =  mktime (3, 0, 0, date("m"), date("d")-3, date("Y") );
  $haTresDias = date("Ymd",$dia);

  $sql = " SELECT pd.codigo_pedido, pd.numero_pedido, pd.data_pedido, pd.valor_total,"; 
  $sql.= " cl.email_cliente, pe.quantidade, pe.quantidade_minima, c.descricao_cor, t.descricao_tamanho";
  $sql.= " FROM tabela_ec_pedidos_detalhes pd, tabela_ec_pedidos_itens pi, tabela_ec_clientes_detalhes cl, tabela_ec_produtos_estoque pe, tabela_ec_produtos_cores c, tabela_ec_produtos_tamanhos t";
  $sql.= " WHERE pd.codigo_cliente=cl.codigo_cliente";
  $sql.= " AND pd.codigo_pedido=pi.codigo_pedido";
  $sql.= " AND pi.codigo_estoque=pe.codigo_estoque";
  $sql.= " AND pi.codigo_cor=c.codigo_cor";
  $sql.= " AND pi.codigo_tamanho=t.codigo_tamanho";
  $sql.= " AND pd.codigo_forma_de_pagamento=4 ";
  $sql.= " AND pd.data_pedido < (now() - INTERVAL 3 DAY)";
  $sql.= " AND pd.codigo_pedido BETWEEN " . $primeiro . " AND " . $ultimo;  
  
  $sql.= " AND pd.codigo_situacao = 2"; 
  $sql.= " AND pd.ativo = 1";

  if(ISSET($_REQUEST['codigo_pedido']))
  {
    $sql.= " AND pd.codigo_pedido = " . $_REQUEST['codigo_pedido'];
  }

  $sql .= " GROUP BY pd.codigo_pedido";
  $sql.= " ORDER BY pd.codigo_pedido DESC";
  ...
  $campos[142] = "Email do Cliente";
  $campos[143] = "varchar";
  $campos[144] = "tabela_ec_clientes_detalhes";
  $campos[145] = "25%";
  $campos[146] = " - ";

  $campos[151] = "valor_total";
  $campos[152] = "Valor";
  $campos[153] = "moeda";
  ...
  $campos[161] = "quantidade";
  $campos[162] = "Quant Atual";
  $campos[163] = "inteiro";
  $campos[164] = "tabela_ec_produtos_estoque";
  $campos[165] = "5%";
  $campos[166] = " - ";

  $campos[171] = "quantidade_minima";
  $campos[172] = "Quant M�nima";
  $campos[173] = "inteiro";
  $campos[174] = "tabela_ec_produtos_estoque";
  $campos[175] = "5%";
  $campos[176] = " - ";

  $campos[181] = "descricao_cor";
  $campos[182] = "Cor";
  $campos[183] = "varchar";
  $campos[184] = "tabela_ec_produtos_cores";
  $campos[185] = "10%";
  $campos[186] = "<br>";

  $campos[191] = "descricao_tamanho";
  $campos[192] = "Tamanho";
  $campos[193] = "varchar";
  $campos[194] = "tabela_ec_produtos_tamanhos";
  $campos[195] = "10%";
  $campos[196] = "<br>";
    ...
    //{	 
      ...
      for($cont=1;$cont<=$numero_campos;$cont++)
      {
        ...
        foreach($campos_para_mostrar AS $nome_do_campo_para_mostrar)
        {
         ...
         if($cont==2)
        {
          echo '<a href="../_modulos/editar_dados.php?codigo_modulo=15&codigo_pedido='.$linha['codigo_pedido'].'" class=relatorio><font color=#eeeeee>';

          /*echo '<a href="../ec_pedidos_detalhes/editar_dados.php?codigo_pedido='.$linha['codigo_pedido'].'" class=relatorio><font color=#eeeeee>';*/
        }
