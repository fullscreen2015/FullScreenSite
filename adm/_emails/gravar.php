<?
  ...
  if (ISSET($_SESSION['senha_result']) && ISSET($_SESSION['login_result']))
  {
    ...
    $consulta = "SELECT " . $chave_primaria ." FROM " . $tabela . " ORDER BY " . $chave_primaria . " DESC LIMIT 1";
  
    $rs_codigo = mysql_query($consulta, $conexao);
    ...
    for($cont=1;$cont<=$numero_campos;$cont++)
    {
      ...
      switch ($campos[$cont3]) 
      {
        ...
        case "moeda":
          ...
          $campos_sql[$cont1] = str_replace(",", ".", $$campos[$cont1]);
          break;
    ...
    for($conta=1;$conta<=$numero_campos_associativos;$conta++)
    {
      ...
      if(ISSET($_REQUEST[$campos_associativos[$conta5]]))
      {
        ...
        for ($i=0;$i<count($_REQUEST[$campos_associativos[$conta1]]);$i++)
        {
          ...
          $sql_gravar = "INSERT INTO " . $campos_associativos[$conta8] . " (".$campos_associativos[$conta91].",".$campos_associativos[$conta9].",".$campos_associativos[$conta5].") VALUES (". $codigo_novo_registro . "," . $$chave_primaria . "," . $array_valores[$i] . ")" ; 
