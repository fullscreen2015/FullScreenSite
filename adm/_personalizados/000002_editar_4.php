<?php
  ...
  // comentei esta linha para não ocorrer o erro quando despublicarmos 
  // um m�dulo. O ideal seria criar um campo ativo para os m�dulos
		...
		echo "<tr valign='middle'>";
			...
			echo "<td width=50% align=left>";
				...
				echo "<font class='preto_8'>";
					...
					echo "<font class='preto_8'><input id='marcacao_sistemas' onClick='marcarTodos(".$maior_codigo_sistema.", ".$maior_codigo_tipo.", this.id)' type='checkbox' /> <b>Marcar/Desmarcar todos</b></font><br><br><br>";
...
<?php
		  ...
		  echo '<font style="margin-left:0px;"><input id="marcacao_sistemas_'.$linha_sistema["codigo_sistema"].'" onClick="marcarTodosDoSistema('.$maior_codigo_tipo.', '.$linha_sistema["codigo_sistema"].', \'marcacao_sistemas\'); checkarCheckboxesDoSistema('.$linha_sistema["codigo_sistema"].', '.$maior_codigo_tipo.', \'marcacao_sistemas\'); checkarCheckboxes('.$maior_codigo_sistema.', '.$maior_codigo_tipo.', \'marcacao_sistemas\');" type="checkbox" /> Todos</font>';
			...
			while($linha_tipo=mysql_fetch_array($rs_tipos))
			{
				...
				$sql = " SELECT * FROM ";
				$sql.= " tabela_adm_ass_usuario_sistema_acesso ";
				$sql.= " WHERE codigo_usuario = " . $_REQUEST['codigo_usuario'];
				$sql.= " AND codigo_sistema=" . $linha_sistema['codigo_sistema'];
				$sql.= " AND codigo_tipo=" . $linha_tipo['codigo_tipo'];
				$rs_dados = mysql_query($sql,$conexao);

				if(mysql_num_rows($rs_dados))
				{
				  $checked = "checked";
				}

				echo ' <input id=marcacao_sistemas_'.$linha_sistema['codigo_sistema'].'_'.$id_checkbox.' onClick="checkarCheckboxesDoSistema('.$linha_sistema["codigo_sistema"].', '.$maior_codigo_tipo.', \'marcacao_sistemas\'); checkarCheckboxes('.$maior_codigo_sistema.', '.$maior_codigo_tipo.', \'marcacao_sistemas\');" '.$checked.' type="checkbox" name=sistema_'.$linha_sistema['codigo_sistema'].'[] value="' . $linha_tipo['codigo_tipo'] . '"> ' . $linha_tipo['descricao_tipo'];
				
				$id_checkbox++;
			}
			
			echo "<script type='text/javascript'>checkarCheckboxesDoSistema(".$linha_sistema['codigo_sistema'].", ".$maior_codigo_tipo.", 'marcacao_sistemas');</script>";
...
<?php
	...
	echo "<script type='text/javascript'>checkarCheckboxes('".$maior_codigo_sistema."', '".$maior_codigo_tipo."', 'marcacao_sistemas');</script>";
  ...
  $sql = " SELECT MAX(codigo_relatorio) as codigo_relatorio ";
  $sql.= " FROM tabela_adm_relatorios";
  $sql.= " WHERE publicar=1 ";//ORDER BY codigo_relatorio DESC LIMIT 1";
  $rs_maior_codigo_relatorio = mysql_query($sql, $conexao);
  $linha_maior_codigo_relatorio = mysql_fetch_array($rs_maior_codigo_relatorio);
  $maior_codigo_relatorio = $linha_maior_codigo_relatorio['codigo_relatorio'];
  $maior_codigo_tipo_relatorio = 1;
...
<?php
							...
							$sql.= " WHERE codigo_usuario = " . $_REQUEST['codigo_usuario'];
							$sql.= " AND codigo_relatorio=" . $linha_relatorio['codigo_relatorio'];
							$rs_dados = mysql_query($sql,$conexao);
...
<?php
		...
		echo "<script type='text/javascript'>checkarCheckboxes('".$maior_codigo_relatorio."', '".$maior_codigo_tipo_relatorio."', 'marcacao_relatorios');</script>";
...
<?php
							...
							$sql.= " WHERE codigo_usuario = " . $_REQUEST['codigo_usuario'];
							$sql.= " AND codigo_grafico=" . $linha_grafico['codigo_grafico'];
							$rs_dados = mysql_query($sql,$conexao);
				...
				<tr valign="middle"> 
					...
					<td width=50% align=left>
						...
						<font class="preto_8">
							...
							<b>Painel - <?php echo $linha_painel['descricao_painel']; ?>:</b>
...
<?php
							...
							$sql.= " WHERE codigo_usuario = " . $_REQUEST['codigo_usuario'];
							$sql.= " AND codigo_painel=" . $linha_painel['codigo_painel'];
							$rs_dados = mysql_query($sql,$conexao);
...
<?php
		...
		echo "<script type='text/javascript'>checkarCheckboxes('".$maior_codigo_painel."', '".$maior_codigo_tipo_painel."', 'marcacao_graficos');</script>";
